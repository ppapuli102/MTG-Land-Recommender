...
# button_style = {"bg": "lightblue", "fg": "black", "font": custom_font}
...
#     # Display the resulting dataframe in separate output widgets
#     for i, col in enumerate(df.columns):
#         col_label = tk.Label(output_frame, text=col, font=custom_font)
#         col_label.grid(row=i, column=0, padx=10, pady=10, sticky="w")
#         col_output = Text(output_frame, wrap=tk.WORD, width=20, height=1)
#         col_output.grid(row=i, column=1, padx=10, pady=10)
#         col_output.insert(tk.END, df[col].to_string(index=False))
...
# # Add input textbox with a label
# input_label = tk.Label(root, text="Input Deck:", font=custom_font)
# input_label.grid(row=1, column=0, padx=10, pady=10, sticky="w")
# input_text = Text(root, wrap=tk.WORD, width=50, height=10)
# input_text.grid(row=2, column=0, padx=10, pady=10)

# # Add "Import" button to import data from the input textbox
# import_button = tk.Button(root, text="Import", command=import_from_textbox, **button
# import pandas as pd
# import numpy as np
# import joblib
# from standardize_io import create_io
...
#     for i, col in enumerate(df.columns):
#         col_label = tk.Label(output_frame, text=col, font=custom_font)
#         col_label.grid(row=i * 2, column=0, padx=10, pady=10)
#         col_output = Text(output_frame, wrap=tk.WORD, width=15, height=10)
#         col_output.grid(row=i * 2 + 1, column=0, padx=10, pady=10)
#         col_output.insert(tk.END, df[col].to_string(index=False))
...
# # Add output frame and label
# output_label = tk.Label(root, text="Output:", font=custom_font)
# output_label.grid(row=6, column=0, padx=10, pady=10, sticky="w")
# output_frame = tk.Frame(root)
# output_frame.grid(row=7, column=0, padx=10, pady=10)
